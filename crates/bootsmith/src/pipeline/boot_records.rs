//! Pure byte-producing functions for the Windows MBR + FAT32 BOOTMGR PBR.
//! The Windows pipeline calls these to compute the bytes it then hands to
...
//!
//! Why not just call bootrec inline from windows.rs?
...
mod tests {
    ...
    //! `splice_pbr_bootmgr`) are bootsmith's entire output surface for
    ...
    //!
    //!     UPDATE_GOLDENS=1 cargo test -p bootsmith --lib boot_records
    //!
    ...
    fn pbr_bootmgr_multi_matches_golden() {
        ...
        // defensively (see bootrec BACKLOG and BOOTSMITH_NTLDR_FINDINGS).
