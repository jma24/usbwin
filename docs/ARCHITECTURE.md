...
3. **Durable.** A workspace of focused crates with explicit traits at module boundaries. The library half (`bootsmith-core`) is testable without root, without a USB stick, and without macOS.

## Workspace layout

```
crates/
├── bootsmith           binary: CLI parsing, top-level orchestration, user prompts
├── bootsmith-core      pipeline types, errors, the WritePlan trait, the four modes
├── bootsmith-iso       ISO9660 inspection + auto-classification
└── bootsmith-disk      Device trait + macOS implementation (DiskArbitration + raw I/O)
...
[`mkmsbr`](https://github.com/jma24/mkmsbr) crate. bootsmith's Cargo.toml
...
1. **Single `DeviceHandle` chokepoint** in `bootsmith-disk`. Every raw write goes through it. It refuses the boot disk, internal disks, and disks > 256 GiB without `--force`.
...
4. **Shell-out is rare and centralized.** Partition tables are bytes, not `fdisk` calls. The only allowed shell-outs are `diskutil unmountDisk` / `mountDisk` / `eject`, wrapped in `bootsmith-disk::macos` with retry + error context.
5. **Test pyramid that doesn't burn USB sticks.**
   ...
   - QEMU smoke: write to a disk image, boot it under qemu-system-i386, scrape serial output. *(Lives in bootrec, not bootsmith — bootrec is where the byte production happens; bootsmith's wrapper is what the golden tests cover.)*
   - Hardware (manual): the scenarios in [`HARDWARE_TESTS.md`](HARDWARE_TESTS.md), run before each release.

## Release targets

**v1.0 is Windows 2000/XP/7.** bootsmith is intentionally not a generic boot
...
`bootmgr` + `install.wim` design, so bootsmith uses GRUB4DOS + FiraDisk instead of the
...
7. **Mount ISO.** `hdiutil attach <iso> -nobrowse -mountpoint /tmp/bootsmith-iso`.
...
`bootsmith-core` is the part that should still compile and run unit tests on Linux/CI, where there's no DiskArbitration framework. Putting macOS-specific code behind a trait in a separate crate forces this discipline. Future v2 work (Linux device backend, GUI frontend) plugs in without touching the core.
